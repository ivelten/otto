...
--
-- The HTTP 'Manager' is injected by the caller so it can be shared with
-- other HTTP clients (notably the AI provider). Sharing a manager gives
-- connection reuse and a single TLS session cache across the process.
--
...
data LoggingConfig = LoggingConfig
  { -- | When 'Just', a @Warning+@-filtered Discord sink is composed into
    -- the 'LogAction'. When 'Nothing', only the stdout sink is active (the
...
bootstrapLogAction :: HTTP.Manager -> LoggingConfig -> IO (LogAction IO Message)
bootstrapLogAction manager cfg = case logDiscordWebhookUrl cfg of
  Nothing -> pure richMessageAction
  Just url -> pure (richMessageAction <> discordSink manager url)
  where
    discordSink :: HTTP.Manager -> Text -> LogAction IO Message
    discordSink mgr url =
...
postDiscord :: HTTP.Manager -> Text -> LogAction IO Message
